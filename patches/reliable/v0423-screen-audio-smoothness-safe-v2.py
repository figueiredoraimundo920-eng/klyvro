...
if source.count(old_display) != 1:
    raise SystemExit(f'v0423 source: fragile display block expected once, found {source.count(old_display)}')
if source.count(old_detail_check) != 1:
    raise SystemExit(f'v0423 source: detail-count guard expected once, found {source.count(old_detail_check)}')
