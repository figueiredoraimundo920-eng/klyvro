-- Klyvro v0.4.11: allow a minimal restart control signal for asymmetric WebRTC failure recovery.
-- The signal contains no media data and is accepted only between two profiles that are actively
-- heartbeating in the same valid voice room.
...
begin
  ...
  if p_kind not in ('description','candidate','screen','restart') then raise exception 'invalid signal kind'; end if;
  ...
  ) then raise exception 'invalid screen signal'; end if;

  if p_kind = 'restart' and (
    jsonb_typeof(p_payload) <> 'object'
    or p_payload <> '{}'::jsonb
  ) then raise exception 'invalid restart signal'; end if;
